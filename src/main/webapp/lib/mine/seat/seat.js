...
seatMapDrawing.prototype = function() {
	...
	var images = new Array(), canvasId = 'seatMapCanvas', ctx = null, rows = 20, seatNumbers = [
			'A', 'B', 'C', 'D', 'E', 'F' ], seat = null, seatsvg = "<svg version='1.1' id='Layer_1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' width='20px' height='20px' viewBox='0 0 20 20' enable-background='new 0 0 20 20' xml:space='preserve'><rect x='0.5' y='0.5' fill='#FFFFFF' stroke='red' stroke-linejoin='round' stroke-miterlimit='20' width='19' height='18'/><rect x='3.5' y='17.5' fill='#FFFFFF' stroke='red' stroke-linejoin='round' stroke-miterlimit='20' width='13' height='2'/><rect x='2.042' y='4.417' fill='none' width='15.688' height='12.729'/><text transform='matrix(1 0 0 1 6.46 12.9365)' font-family='MyriadPro-Regular' color='red' style='display:none;' font-size='12'>X</text></svg>",
	...
	drawSeatMap = function(el, currentSeat) {
		...
		el.empty();
		var canvas = el.append('<canvas id="' + canvasId + '" height="' + h
				+ '" width="' + w + '"></canvas>');
	...
	getImageForSeat = function(row, num) {
		...
		var lsvg = seatsvg;
		if (row != 5) {
			lsvg = lsvg.replace('red', 'rgba(127,127,127,.4)').replace('red',
					'rgba(127,127,127,.4)').replace('display:none',
					'display:block');
			ret.state = 'occupied';
		} else {
			ret.state = 'available';
